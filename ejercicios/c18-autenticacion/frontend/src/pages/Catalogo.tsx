...
export default function Catalogo({ libros }: { libros: Libro[] }) {
  ...
  const { data: librosCargadosBackend, loading, error } = useFetch<BackendLibro[]>('/libros');
  const librosCargados: Libro[] | null = librosCargadosBackend
    ? librosCargadosBackend.map((libro) => ({
        id: libro.id,
        title: libro.titulo,
        author: libro.autor,
        img: libro.imagen,
        alt: libro.titulo,
        description: '',
        price: `$${libro.precio.toLocaleString('es-AR')}`,
  ...
  if (loading && librosBase.length === 0) {
    return (
